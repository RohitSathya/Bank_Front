...
export default function AllUsers() {
    ...
    useEffect(() => {
        async function fetchUser() {
            try {
                const response = await axios.get(link+'api/admin/getuser');
    ...
    async function removeUser(id) {
        try {
            const res2 = await axios.delete(link+`api/admin/removeuser/${id}`);
            const { message } = res2.data;
            if (message === 's') {
                const response3 = await axios.get(link+'api/admin/getuser');
    ...
    async function saveEdit() {
        try {
            ...
            const res = await axios.post(link+'api/auth/edituser', {
                uid: editUserId,
    ...
    async function active(e,s){
        if(s=='inactive'){
            ...
            console.log('dfdf')
            const res=await axios.post(link+'api/auth/active',{id:e,s:'active'})
            ...
            const res=await axios.post(link+'api/auth/active',{id:e,s:'inactive'})
            const response = await axios.get(link+'api/admin/getuser');
                const { message, ud } = response.data;
                if (message === 'f') {
                    toast.error('No users available');
