...
export default function ChatUsers() {
    ...
    useEffect(()=>{
       ...
       async function viewchat(e){
        ...
        m1.style.display='block'
         const res=await axios.get(link+`api/chat/getmsg/${e}`)
         const {m}=res.data
         if(m=='f'){
            sf(0)
            setmessage([])
 ...
 async function sendMessage(){
    ...
    const res=await axios.post(link+'api/chat/sendmsg',{userId:'Admin',message:inputMessage,to:to})
    const {m}=res.data
    if(m=='s'){
        
    setInputMessage(''); 
    inputMessageRef.current.focus(); 
    const res2=await axios.get(link+`api/chat/getmsg/${to}`)
