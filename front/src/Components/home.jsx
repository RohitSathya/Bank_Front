...
export default function Home({ func }) {
    ...
    useEffect(() => {
        async function g() {
            ...
            const response = await axios.get(link+`api/transaction/balance/${parse._id}`);
    ...
    const handleDeposit = async (action) => {
        ...
        if (action === 'deposit') {
            ...
            const res1=await axios.post(link+'api/transaction/deposit',{accountno:parse.accountno,balance:am})
            const {bal}=res1.data
            setBalance(bal);
            ...
            if (balance === 0) {
                ...
                const res2=await axios.post(link+'api/transaction/withdraw',{accountno:parse.accountno,balance:am2})
            const {bal}=res2.data
            setBalance(bal);
            toast.success('Deposited successfully');
            }
    ...
    const handleMessageSend = async() => {
        if (message.trim() !== '') {
          const ud=localStorage.getItem('userdetail')
          const parse=JSON.parse(ud)
          const res=await axios.post(link+'api/chat/sendmsg',{userId:parse._id,message:message,to:'Admin'})
          const {m}=res.data
          if(m=='s'){
            setMessage('')
    ...
    async function getbalance() {
        ...
        const userDetail = localStorage.getItem('userdetail');
        const parse = JSON.parse(userDetail);
        const response = await axios.get(link+`api/transaction/balance/${parse.accountno}`);
        const { balance } = response.data;
        setBalance(balance);
        a.style.display = 'block';
    }
    ...
    useEffect(()=>{
       ...
       async function viewchat(e){
        ...
        m1.style.display='block'
         const res=await axios.get(link+`api/chat/getmsgU/${e}`)
         const {m}=res.data
         if(m=='f'){
            sf(0)
            setmessages([])
       ...
       async function sendMessage(){
        ...
        const res=await axios.post(link+'api/chat/sendmsg',{userId:uid,message:inputMessage,to:'Admin'})
        const {m}=res.data
        if(m=='s'){
        ...
        const res2=await axios.get(link+`api/chat/getmsgU/${uid}`)
