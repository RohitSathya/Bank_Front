...
export default function loanapproval() {
    ...
    useEffect(()=>{
        async function loanstat(){
            ...
            const res=await axios.get(link+'api/transaction/loan-approval')
    ...
    async function reject(id){
        const res=await axios.delete(link+`api/transaction/loan-reject/${id}`)
    ...
    async function accept(id,amount,uid){
        console.log(id)
        const res=await axios.get(link+`api/transaction/loan-approve/${id}`)
        const {message}=res.data
        if(message=='s'){
            const res2=await axios.post(link+`api/transaction/deposit`,{accountno:uid,balance:amount})
